...
trait NotTwoConsecutive[T] extends Parser[T]{ // one cannot parse two consecutive elements which are equal
  ...
  abstract override def end(): Boolean = {consecutive = true; super.end()}
}
